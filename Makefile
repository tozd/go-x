...
.PHONY: test test-ci lint lint-ci fmt fmt-ci upgrade clean lint-docs lint-docs-ci audit encrypt decrypt sops
...
fmt:
	...
	git ls-files --cached --modified --other --exclude-standard -z | grep -z -Z '.go$$' | xargs -0 goimports -w -local gitlab.com/tozd/go/x
